...
if($error == false){
    move_uploaded_file($tmpName_photo, '../view/assets/img/'.$name_photo);
        ...
        $sql = "UPDATE projet SET name=:name, description=:description, annee=:annee, temps=:temps, media=:media,link=:link WHERE id_projet=$id_projet"; // met à jour les données dans la tavle lorsque code client est égal à l'id ( il faut l'auto incrément )
